...
An example for this coordinate system can be found in [examples/coordinate_system](https://github.com/FAIRmat-NFDI/pynxtools-xps/tree/main/examples/coordinate_system).
...
```console
pynx convert config_vms_cs_fixed.json --reader xps --nxdl NXxps --ignore-undocumented --output vms-cs-fixed.nxs
```
...
```console
uv pip install nexus3d
python shapes/shapes.py
nexus3d vms-cs-fixed.nxs -fo vms-cs.glb --blender -c config-stl.json -s 1
```