...
The example conversion can be run with the following command.
```console
user@box:~$ dataconverter In-situ_PBTTT_XPS_SPECS.xml eln_data.yaml --reader xps --nxdl NXxps --output In-situ_PBTTT.nxs
```
...
The example conversion can be run with the following command.
```console
user@box:~$ dataconverter MgFe2O4.xy eln_data.yaml --reader xps --nxdl NXxps --output MgFe2O4.nxs
``` 