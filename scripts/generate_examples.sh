...
function update_cs_example {
  ...
  pynx convert config_vms_cs_fixed.json \
...
function update_phi_examples {
  ...
  pynx convert SnO2_10nm_1.pro eln_data_phi.yaml \
    --reader $READER --nxdl $NXDL \
    "${DATACONVERTER_EXTRA_ARGS[@]}" \
...
function update_scienta_examples {
  ...
  pynx convert Cu-HHTP_*.ibw eln_data_scienta_ibw.yaml \
    --reader $READER --nxdl $NXDL \
    "${DATACONVERTER_EXTRA_ARGS[@]}" \
  ...
  pynx convert Cu-HHTP_*.txt eln_data_scienta_txt.yaml \
    --reader $READER --nxdl $NXDL \
    "${DATACONVERTER_EXTRA_ARGS[@]}" \
...
function update_specs_examples {
  ...
  pynx convert EX439_S718_Au.sle eln_data_sle.yaml \
    --reader $READER --nxdl $NXDL \
    "${DATACONVERTER_EXTRA_ARGS[@]}" \
...
function update_vms_examples {
  ...
  pynx convert irregular.vms eln_data_vms.yaml \
    --reader $READER --nxdl $NXDL \
    "${DATACONVERTER_EXTRA_ARGS[@]}" \
  ...
  pynx convert vms_txt_export.txt eln_data_vms_txt_export.yaml \
    --reader $READER --nxdl $NXDL \
    "${DATACONVERTER_EXTRA_ARGS[@]}" \
