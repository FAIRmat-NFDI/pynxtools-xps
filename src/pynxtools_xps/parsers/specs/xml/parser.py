...
_KEY_PATTERNS = [re.compile(rf"{key_part}(.*?)(?=\/|$)") for key_part in _KEY_PARTS]


def _construct_entry_name(key: str) -> str:
    """Construct entry name."""
    name_parts = []

    for key_pattern in _KEY_PATTERNS:
        match = re.search(key_pattern, key)
        if match:
            name_part = _align_name_part(match.group(1))
...
class SpecsXMLParser(_XPSParser):
    ...
    def parse_file(self, file: str | Path, **kwargs):
        ...
        for key, value in self.metadata_dict.items():
            entry_name = _construct_entry_name(key)
            str_to_replace = "/"
            for i, key_pattern in enumerate(_KEY_PATTERNS):
                match = re.search(key_pattern, key)
                if match:
